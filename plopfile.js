...
const { extensions, projects } = require('./package-constants')
...
module.exports = (plop) => {
  let extSplit =
    config.extension && config.extension.includes('x')
      ? config.extension.split('x')[0]
      : config.extension
  ...
  plop.setGenerator('project', {
    prompts: [
      ...
      },
	  {
        type: 'list',
        name: 'isTestSeperate',
        default: plopConfig.defaultTestSeperate,
        message: 'You want a to keep your unit test case in tests folder?',
        choices: () => plopConfig.availableseperateFolderList(),
      },
    ],
    actions: [
      {

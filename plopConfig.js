...
const getConfig = async () => {
  let data = await fs.readFileSync("./node_modules/react-plop-template/config.json", "utf-8");
  data = data && JSON.parse(data.toString());
  return data;
};
const saveConfig = async (data) => {
  data = JSON.stringify(data, null, 2);
  await fs.writeFileSync("./node_modules/react-plop-template/config.json", data);
};
module.exports = {
